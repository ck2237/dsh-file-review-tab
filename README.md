# dsh-file-review-tab (0.1.2-alpha.1 compatibility fork)

> 本包基于 `dsh-file-review-tab@0.4.1` 的已发布产物做最小兼容补丁，用于 **DeepSeek Harness 0.1.2-alpha.1**。
>
> 补丁内容：
> 1. 按当前 DSH 真实快照结构（`call.argsRaw` + `meta.diffs`）重建「文件审查」两个入口的文件/差异识别；
> 2. 侧边栏 Tab 与角标的快照源改为 `uiConversation.binding(sessionId).snapshot`（0.1.2-alpha.1 的会话控制器 `session.getSnapshot()` 不含聊天节点）。
>
> 依赖：`dsh-better-sidebar`（≥0.12.0）。
>
> 安装（GitHub）：
> ```sh
...
> ```
>
> 注意：发布前请把 `package.json` 里的 `repository` 改成你的仓库地址。
